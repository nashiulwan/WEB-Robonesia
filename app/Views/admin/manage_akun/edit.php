...
<div class="container-fluid">
    ...
    <form action="<?= base_url('admin/manage_akun/update/' . $users['id']) ?>" method="post" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <input type="text" name="username" id="username" class="form-control <?= (session()->getFlashdata('errors') && array_key_exists('username', session()->getFlashdata('errors'))) ? 'is-invalid' : '' ?>" value="<?= old('username') ?>" placeholder="Masukkan nama pengguna" required autocomplete="on">
        ...
        <div class="mb-3">
            ...
            <?php endif; ?>
            <?php if (!empty($users['gambar'])): ?>
                <p>Gambar saat ini:</p>
                <img src="<?= base_url('/uploads/' . $users['gambar']) ?>" alt="Gambar Artikel" width="150">
            <?php endif; ?>
        </div>
        ...
        <div class="form-group">
            ...
            <select class="custom-select" name="status" required>
                <option value="1" <?= (old('status') == '1' || $users['status'] == '1') ? 'selected' : '' ?>>Aktif</option>
                <option value="0" <?= (old('status') == '0' || $users['status'] == '0') ? 'selected' : '' ?>>Tidak Aktif</option>
            </select>
        </div>
