...
<div class="container-fluid">
  ...
  <form action="<?= base_url('guru/grade_level/proyek/simpan/' . $kelas['id']) ?>" method="post" enctype="multipart/form-data">
    <?= csrf_field() ?>
    ...
    <button type="submit" class="btn btn-primary mt-3" style="width:7rem">Simpan</button>
    <a href="<?= base_url('guru/grade_level/proyek/' . $kelas['id']) ?>" class="btn btn-warning mt-3" style="width:7rem">Kembali</a>
  </form>
</div>
    ...
    if (cropper) {
      ...
      if (canvas) {
        canvas.toBlob(blob => {
          ...
          // Hapus atau komentar baris berikut agar tombol rasio tetap terlihat
